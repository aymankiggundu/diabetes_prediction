...
def knowledge_distillation(teacher_model, X_train, y_train, X_test, temperature=2):
    ...
    student_model = svm.SVC(kernel='linear', probability=True)
...
if __name__ == "__main__":
    ...
    training_time = svm_model.train(X_train, y_train)
    
    # Evaluate the model
    accuracy, precision, recall, f1, auc, sensitivity = svm_model.evaluate(X_test, y_test)
    ...
    training_time_denoised = svm_model.train(X_train_denoised, y_train)
    accuracy_denoised, precision_denoised, recall_denoised, f1_denoised, auc_denoised, sensitivity_denoised = svm_model.evaluate(X_test_denoised, y_test)
    ...
    student_model = knowledge_distillation(svm_model.model, X_train, y_train, X_test)
    ...
    sparse_kd_model = sparse_knowledge_distillation(svm_model.model, X_train, y_train, X_test)
    ...
    student_model_denoised = knowledge_distillation(svm_model.model, X_train_denoised, y_train, X_test_denoised)
    ...
    sparse_kd_model_denoised = sparse_knowledge_distillation(svm_model.model, X_train_denoised, y_train, X_test_denoised)
