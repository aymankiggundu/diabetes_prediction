...
def knowledge_distillation(teacher_model, X_train, y_train, X_test, temperature=2):
    ...
    student_model = KNeighborsClassifier(n_neighbors=5)
...
if __name__ == "__main__":
    ...
    training_time = knn_model.train(X_train, y_train)
    
    # Evaluate the model
    accuracy, precision, recall, f1, auc, sensitivity = knn_model.evaluate(X_test, y_test)
    ...
    training_time_denoised = knn_model.train(X_train_denoised, y_train)
    accuracy_denoised, precision_denoised, recall_denoised, f1_denoised, auc_denoised, sensitivity_denoised = knn_model.evaluate(X_test_denoised, y_test)
    ...
    student_model = knowledge_distillation(knn_model.model, X_train, y_train, X_test)
    ...
    sparse_kd_model = sparse_knowledge_distillation(knn_model.model, X_train, y_train, X_test)
    ...
    student_model_denoised = knowledge_distillation(knn_model.model, X_train_denoised, y_train, X_test_denoised)
    ...
    sparse_kd_model_denoised = sparse_knowledge_distillation(knn_model.model, X_train_denoised, y_train, X_test_denoised)
